...
const Header_Category = () => {
    ...
    // Default value [] rakhi hai taaki .length crash na kare
    ...
    const get_categories = async () => {
        try {
            ...
            // Agar data.categories undefined ho toh empty array set ho jaye
        ...
        <div className='w-full'>
            <div className='bg-[#c80000] w-full text-white uppercase font-semibold relative'>
                <div className='px-8 flex justify-between items-center relative h-[48px]'>
                    ...
                    {/* Desktop Menu - Yahan error aa raha tha */}
                    <div className='flex-wrap hidden lg:flex'>
                        <Link className={`px-6 font-medium py-[13px] ${path === '/' ? 'bg-[#00000026]' : ''}`} href={'/'} >Home</Link>
                        {
                            ...
                            categories?.length > 0 && categories.map((c, i) => (
                                <Link 
                                    key={i} 
                                    className={`px-6 font-medium py-[13px] ${path === c.category ? 'bg-[#00000026]' : ''}`} 
            ...
            {/* Mobile Menu */}
            {
                cate_show && <div className='flex flex-wrap lg:hidden py-2 px-[30px] bg-[#c80000] text-white'>
