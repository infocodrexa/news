...
const Edit_news = () => {
    ...
    // 👇 NEW: SubCategory ke liye Slug State
    ...
    // 👇 NEW: Handle SubCategory Change (Auto Generate Slug)
    const handleSubCategoryChange = (e) => {
        const val = e.target.value;
        setSubCategory(val);
        setSubCategorySlug(generateSlug(val)); // Auto-generate slug
    };
    ...
    const get_news = async () => {
        try {
            ...
            // 👇 SubCategory aur uska Slug fetch karo
            setSubCategory(data?.news?.subCategory || '');
            setSubCategorySlug(data?.news?.subCategorySlug || ''); 
            
            setTags(data?.news?.tags || '');
            set_old_image(data?.news?.image);
            
            if(data?.news?.image) {
                if(data.news.image.includes('http')) {
                    setImg(data.news.image);
                } else {
                    setImg(`${base_url}/uploads/${data.news.image}`);
                }
            }
        } catch (error) {
            console.log(error);
    ...
    const update = async (e) => {
        ...
        if (!description.trim()) { toast.error("Content description cannot be empty!"); return; }
        ...
        // 👇 SubCategory aur uska Slug bhejo
    ...
    return (
        ...
        >
            ...
            <form onSubmit={update} className='grid grid-cols-1 lg:grid-cols-12 gap-8'>
                ...
                <div className='lg:col-span-4 space-y-6'>
                    ...
                    <div className='bg-gray-50 p-5 rounded-xl border border-gray-200'>
                        <h3 className='font-bold text-gray-700 mb-4 border-b pb-2'>Actions</h3>
                        <div className='text-sm text-gray-600 flex flex-col gap-3'>
                            <div className='flex justify-between'>
                                <span>Current Status:</span><span className='font-bold text-orange-500'>Editing</span>
                            </div>
                        </div>
                        <button disabled={loader} onClick={update} className='w-full mt-6 py-2.5 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 transition shadow-md disabled:opacity-70 flex justify-center items-center gap-2'>
                            {loader ? <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span> : 'Update News'}
                        </button>
                    </div>
                        ...
                        <div className='flex flex-col gap-3'>
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={subCategory}
                                    onChange={handleSubCategoryChange} // Auto Slug generate hoga
